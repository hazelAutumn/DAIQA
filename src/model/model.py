...
###### Output 2 output, but only train with the IQA loss
class ModelDis:
    def __init__(self, cfg, net_arch, loss_f, rank=0):
        # prepare var
        self.cfg = cfg
    ...
    def optimize_parameters(self, model_input, model_target, model_target2):
        self.net.train()
        self.optimizer.zero_grad()
        output, output2 = self.run_network(model_input)
        loss_v = self.compute_loss(output, model_target)
        loss_v.backward()
        self.optimizer.step()
        self.scheduler.step()
    ...
    def compute_loss(self, model_output, model_target):
        loss_v = 0.0
        # Here is for training only IQA part
        #loss_v = self.loss_f[0][1] * self.loss_f[0][0](model_output, model_target.unsqueeze(1).to(self.device))
        # Below is for jointraining
        #loss_v = self.loss_f[0][1] * self.loss_f[0][0](model_output, model_target.unsqueeze(1).to(self.device)) #+ \
        #            self.loss_f[1][1] * self.loss_f[1][0](model_output2, model_target2.to(self.device).long()) 
        for fn, weight in self.loss_f:
            loss_v += weight * fn(
                model_output, model_target.unsqueeze(1).to(self.device)
            )
...
###### Output 2 output, this is for joint training, the compute_loss considers both 2 outputs
class ModelDis_original:
    def __init__(self, cfg, net_arch, loss_f, rank=0):
        # prepare var
        self.cfg = cfg
        self.device = self.cfg.dist.device
    ...
    def optimize_parameters(self, model_input, model_target, model_target2):
        ...
        self.optimizer.zero_grad()
        output, output2 = self.run_network(model_input)
        loss_v = self.compute_loss(output, output2, model_target, model_target2)
        loss_v.backward()
        self.optimizer.step()
        self.scheduler.step()
        # set log
    ...
    def compute_loss(self, model_output, model_output2, model_target, model_target2):
        loss_v = 0.0
        # Here is for training only IQA part
        #loss_v = self.loss_f[0][1] * self.loss_f[0][0](model_output, model_target.unsqueeze(1).to(self.device))
        # Below is for jointraining
...
class ModelDisOnly:
    ...
    def optimize_parameters(self, model_input, model_target2):
        self.net.train()
        self.optimizer.zero_grad()
        output2 = self.run_network(model_input)
        loss_v = self.compute_loss(output2, model_target2)
        loss_v.backward()
        self.optimizer.step()
        self.scheduler.step()
        # set log
    ...
    def compute_loss(self, model_output2, model_target2):
        loss_v = 0.0
        #loss_v = self.loss_f[0][1] * self.loss_f[0][0](model_output2, model_target2.to(self.device).long()) 
        for fn, weight in self.loss_f:
            loss_v += weight * fn(
                model_output2, model_target2.to(self.device).long()
            )
        return loss_v
...
###### Output 2 output, but only train with the Distortion loss
class ModelDis2:
    def __init__(self, cfg, net_arch, loss_f, rank=0):
        # prepare var
        self.cfg = cfg
    ...
    def compute_loss(self, model_output, model_output2, model_target, model_target2):
        ...
        for fn, weight in self.loss_f:
            loss_v += weight * fn(
                model_output2, model_target2.unsqueeze(1).to(self.device)
            )
        return loss_v
