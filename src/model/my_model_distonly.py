...
"""
#v1 of but only predict distortion classification
...
class Learner(nn.Module):
    def __init__(self, cfg):
        ...
        num_distortions = cfg.model.learner_param.num_distortions
        ...
        self.up_proj_cnn = Mlp(
             in_features=grid_size*grid_size*cnn_feature_num+12,
        ...
        self.mlp_distortion = Mlp(
             in_features=grid_size*grid_size*cnn_feature_num,
             hidden_features=latent_dim,
             out_features=latent_dim,
        )
        ...
        self.head_quality = nn.Sequential(
            ...
            nn.GELU(), 
            NormedLinear(latent_dim, num_classes)
        )
        self.head_distortion = nn.Sequential(
            nn.Linear(latent_dim*latent_dim, latent_dim), 
            nn.GELU(), 
            nn.Linear(latent_dim, num_distortions),
            nn.LogSoftmax(dim=1)
        )
    ...
    def forward(self,x, y): #y:N, latent_dim, 7x7x4 # x: #N, 49x4+12 , latent_dim
        x = x.permute(0,2,1) #x: #N,  latent_dim, 49x4+12
        x = self.up_proj_cnn(x) #N, latent_dim, latent_dim #video: N*frames, latent_dim, laten_dims
        x = self.head_quality(torch.flatten(x, 1)) # video: N*frames, 1
        y = self.mlp_distortion(y) #N, latent_dim, latent_dim
        y = self.head_distortion(torch.flatten(y, 1))
        return x,y
    


class MyModel_v1_0(VisionTransformer):
    def __init__(
        self,
        cfg=None,
    ...
    def forward_features(self, x, cnn_tokens):
        ...
        for i in range(6): #range(len(self.blocks)):
            x = self.blocks[i](x)
        ...
        cls_feat = x[:,0,:] #N,1,C
        cls_feat = cls_feat.reshape(batch_size, -1, latent_dim) #N,  , latent_dim
        #print("cls shape ", cls_feat.shape)
        x = torch.cat((cls_feat, cnn_tokens), dim=1) #N, 49x4+ , 64 #check here, add distortion token
        
        #x = self.norm(x)
        return x

    def forward(self, x):
        cnn_tokens = self.forward_hyper_net(x) #N,7x7x4, latent_dim
        x = self.forward_features(x, cnn_tokens) #N, 49x4+ , latent_dim
        x = self.dropout(x)
        cnn_tokens = cnn_tokens.permute(0, 2, 1) #N, latent_dim, 7x7x4
        x, y = self.learner(x,cnn_tokens)
        x = self.head(x)
        return y


# Only quality prediction, using the pretrained from distortion classification
class MyModel_v1_4(VisionTransformer):
    def __init__(
        self,
        cfg=None,
        embed_layer=PatchEmbed,
    ...
    def freeze(self):
        ...
        # Aldo need to freeze learner.conv.paramters()
        for param in self.learner.head_distortion.parameters():
            param.requires_grad = False
        for param in self.learner.mlp_distortion.parameters():
            param.requires_grad = False
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        cnn_tokens = cnn_tokens.permute(0, 2, 1) #N, latent_dim, 7x7x4
        x, _ = self.learner(x, cnn_tokens)
        x = self.head(x)
        return x


# Using the pretrained from distortion classification, train only IQA, but output both IQA and distortion labels
class MyModel_v1_42(VisionTransformer):
    def __init__(
        self,
        cfg=None,
        embed_layer=PatchEmbed,
    ...
    def freeze(self):
        ...
        for param in self.learner.parameters():
            param.requires_grad = False
        for param in self.learner.head_quality.parameters():
            param.requires_grad = True
        for param in self.learner.up_proj_cnn.parameters():
            param.requires_grad = True
