...
"""
#v1 of using distortion label and pretrain on a synthetic dataset
...
class Learner(nn.Module):
    ...
    def forward(self, x, y): #y:N, latent_dim, 7x7x4 # x: #N, 49x4+12 , latent_dim
        ...
        x = self.up_proj_cnn(x) #N, latent_dim, latent_dim
        x = self.head_quality(torch.flatten(x, 1))
        y = self.mlp_distortion(y)
        y = self.head_distortion(torch.flatten(y, 1))
        return x, y


class MyModel_v1_2(VisionTransformer):
    def __init__(
        self,
        cfg=None,
