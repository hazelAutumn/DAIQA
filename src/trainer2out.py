# Two ouputs, for training quality prediction and distortion distortion at the same time
import datetime
import itertools
import os
import random
...
from src.model import create_model, ModelDis
from src.tools import train_model2, test_model2
...
def train_loop(rank, cfg):
    ...
    model = ModelDis(cfg, net_arch, loss_f, rank)

    #Log number of parameter in the networks
    num_params = sum(param.numel() for param in net_arch.parameters())
    logger.info("Number of parameters in the model: %d"  % (num_params) )

    trainable_params = sum(p.numel() for p in net_arch.parameters() if p.requires_grad)
    logger.info("Number of fine-tunning parameters in the model: %d"  % (trainable_params) )
    ...
    # DEBUG
    # logger.info("========================  conv")
    # for param in model.net.learner.conv.parameters():
    #     logger.info(param)

    # logger.info("========================    head_distortion")
    # for param in model.net.learner.head_distortion.parameters():
    #     logger.info(param)

    # logger.info("========================    mlp_distortion")
    # for param in model.net.learner.mlp_distortion.parameters():
    #     logger.info(param)
